...
return class Test
  ...
  _get_envs: (t_names, g_names) ->
    envs.get(name) for name in set.union(t_names, g_names)
  ...
  measure: ->
    ...
    arr = (@run(arg) for i in [1..settings.MEASURE_RETRY])
    
    @_arg = settings.EXECUTE_LIMIT / math.avg(arr) * arg
